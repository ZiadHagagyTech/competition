...
// تحديث: عرض رسالة نجاح بعد الحفظ في قاعدة البيانات (تم إصلاح سبب عدم ظهور الرسالة)
// التعديلات الرئيسية:
// - أزلت مسح رسالة الحالة من معالج الحدث reset حتى لا تُمسح رسالة النجاح فورًا.
// - بعد الحفظ، أقوم بعمل form.reset() ثم إعادة عرض رسالة النجاح بعد تأخير قصير (للتغلب على أي معالجات أخرى).
// - باقي الميزات السابقة (تحجيم الصور، رفع إلى Imgbb/Firebase/DataURL، تفرد الرقم القومي، حقل المكتب، قيود المدخلات، قواعد المستويات...) محفوظة.
...
(function () {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // ملاحظة مهمة: في هذه النسخة لا نمسح رسالة الحالة عند تنفيذ reset،
    // لأن ذلك كان يزيل رسالة النجاح فورًا بعد حفظ السجل.
    form.addEventListener('reset', () => {
      setTimeout(() => {
        ...
        clearBirthError();
        clearLevelError();
        // لا نمسح showMessage هنا — رسالة النجاح ستُعرض بعد reset من ��الة الحفظ
    ...
    // ========= قيود طول المدخلات الرقمية =========
    function enforceDigitLimit(inputEl, maxLen) {
      if (!inputEl) return;
      inputEl.setAttribute('maxlength', String(maxLen));
      inputEl.addEventListener('input', () => {
        const orig = inputEl.value || '';
        const cleaned = orig.replace(/\D+/g, '').slice(0, maxLen);
        if (cleaned !== orig) inputEl.value = cleaned;
      });
      inputEl.addEventListener('paste', (e) => {
        ...
        const newVal = (current.slice(0, start) + cleaned + current.slice(end)).replace(/\D+/g, '').slice(0, maxLen);
        inputEl.value = newVal;
        const pos = Math.min(start + cleaned.length, maxLen);
        try { inputEl.setSelectionRange(pos, pos); } catch (e) {}
        inputEl.dispatchEvent(new Event('input', { bubbles: true }));
      });
    }

    enforceDigitLimit(nationalIdInput, 14);
    enforceDigitLimit(guardianPhoneInput, 11);
    enforceDigitLimit(whatsappInput, 11);
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (vErr) {
        ...
        if (/هاتف/.test(vErr)) setFieldError(guardianPhoneInput, vErr);
        return;
      }

      // تحقق من تفرد الرقم القومي قبل أي رفع للملفات (لتجنب رفع ملفات ثم إلغاء)
      ...
      try {
        ...
        if (!photoUrl && photoFile) throw new Error('فشل رفع الصورة الشخص��ة. حاول مرة أخرى.');
        ...
        // إنشاء فهرس للرقم القومي (مساعد للبحث مستقبلاً) - غير حرج
        try {
        ...
        // الآن نعيد تعيين النموذج أولًا (سيتم تنفيذ معالج reset أعلاه لكنه لا يمسح الرسالة الآن)
        form.reset();

        // ثم نعرض رسالة نجاح واضحة بعد تأخير بسيط للتأكد من أنها تظهر بعد أي عمليات أخرى
        setTimeout(() => {
          showMessage(`تم تسجيل المتسابق بنجاح. رقم مفتاح السجل: ${saveResult.key}`, 'green');
        }, 150);
