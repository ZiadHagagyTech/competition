// admin.js — لوحة تحكم القائد
// التعديلات:
// - استخدمت مسار `scores/{username}` بدلاً من `results/{username}` لقراءة/حفظ درجات المتسابقين.
// - جعلت الحفظ قابلًا للتعديل: يمكنك تعديل الدرجات لاحقًا (لن يتم تعطيل زر الحفظ نهائياً).
// - حفظ الحقول: hifzScore, ahkamScore, total, gradedBy, gradedAt. إذا كان سجل سابق يحتوي على `finalized` نحتفظ به.
// - تحديث دوال التصدير لاستخدام مسار `scores` وإضافة عمود "المكتب" كما طلبت سابقًا.
...
(function(){
  ...
  exportAllBtn && exportAllBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      notice('تم تنزيل ملف Excel لجميع الشيوخ', 'green');
    } catch (err) {
      console.error(err);
  ...
  // Toggle competitors panel (fetch from competitors/{username}) — now uses scores path
  async function toggleCompetitorsPanel(rowEl, username) {
    ...
    try {
      // fetch competitors + scores + sheikh profile
      const [compsSnap, scoresSnap, sheikhSnap] = await Promise.all([
        firebase.database().ref(`competitors/${username}`).once('value'),
        firebase.database().ref(`scores/${username}`).once('value'),
        firebase.database().ref(`sheikhs/${username}`).once('value')
      ]);
      const sheikhProfile = sheikhSnap.exists() ? sheikhSnap.val() : { username };
      const scoresObj = scoresSnap.exists() ? scoresSnap.val() : {};
      const comps = [];
      if (compsSnap.exists()) {
        compsSnap.forEach(child => {
          ...
          const office = (val && (val.office || val.officeName || val['المكتب'] || val['office'])) || '';
          comps.push({ key, fullname, region, level, office, raw: val });
        });
      }
      ...
      html.push(`</div>`);
      html.push(`<table class="compact-table"><thead><tr><th>م</th><th>المتسابق</th><th>المنطقة</th><th>المكتب</th><th>المستوى</th><th>حفظ /75</th><th>أحكام /25</th><th>الإجمالي</th><th>إجراءات</th></tr></thead><tbody>`);

      for (let i=0;i<comps.length;i++){
        const c = comps[i];
        const sc = scoresObj && scoresObj[c.key] ? scoresObj[c.key] : null;
        const h = sc && sc.hifzScore != null ? sc.hifzScore : '';
        const a = sc && sc.ahkamScore != null ? sc.ahkamScore : '';
        const total = (h === '' && a === '') ? '' : (Number(h || 0) + Number(a || 0));
        const btnLabel = sc ? 'تعديل' : 'حفظ';
        const btnClass = sc ? 'btn-saved' : '';
        // Note: button is enabled always to allow editing
        html.push(`<tr data-comp-key="${escapeAttr(c.key)}">
          ...
          <td data-label="المكتب">${escapeHtml(c.office || '')}</td>
          ...
          <td data-label="إجراءات"><button class="small-btn ${btnClass}" data-save-key="${escapeAttr(c.key)}">${escapeHtml(btnLabel)}</button></td>
        </tr>`);
      }
      ...
      inner.addEventListener('click', async (ev) => {
        ...
        // allow saving/editing always (no permanent disable)
  ...
  // Save single result to `scores/{username}/{compKey}` — editable
  async function saveSingleResult(username, compKey, panelEl, btnEl) {
    // temporary disable while saving
    btnEl.disabled = true;
    ...
    try {
      ...
      // preserve existing finalized flag if present
      const existingSnap = await firebase.database().ref(`scores/${username}/${compKey}`).once('value');
      const existing = existingSnap.exists() ? existingSnap.val() : null;
      const finalized = existing && typeof existing.finalized !== 'undefined' ? existing.finalized : false;
      ...
      const payload = {
        ...
        finalized: finalized // keep previous finalized state
      };

      await firebase.database().ref(`scores/${username}/${compKey}`).set(payload);

      // after save: show saved state but allow further edits
      btnEl.textContent = 'تعديل';
  ...
  // Save all (bulk) to scores/{username}/{compKey} — editable
  async function saveAllForSheikh(username, panelEl) {
    const rows = Array.from(panelEl.querySelectorAll('tr[data-comp-key]'));
    if (rows.length === 0) return;
    // fetch existing scores to preserve finalized flags
    const scoresSnap = await firebase.database().ref(`scores/${username}`).once('value');
    const scoresObj = scoresSnap.exists() ? scoresSnap.val() : {};
    ...
    for (const row of rows) {
      ...
      const existing = scoresObj && scoresObj[compKey] ? scoresObj[compKey] : null;
      const finalized = existing && typeof existing.finalized !== 'undefined' ? existing.finalized : false;
      updates[`scores/${username}/${compKey}`] = {
        hifzScore: h,
        ahkamScore: a,
        total: ((h != null ? h : 0) + (a != null ? a : 0)),
        gradedBy: getAdminSession() ? getAdminSession().username : 'admin',
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (btn) { btn.textContent = 'تعديل'; btn.disabled = false; btn.classList.add('btn-saved'); }
  ...
  // Export single sheikh as Excel (columns: index, name, region, office, level, hifz, ahkam, total)
  // now reads scores path
  async function exportSheikh(username) {
    ...
    let tableHtml = `
      ...
      <table>
        ...
        <thead><tr>
          <th>م</th><th>اسم المتسابق</th><th>المنطقة</th><th>المكتب</th><th>مستوى الحفظ</th><th>حفظ /75</th><th>أحكام /25</th><th>الإجمالي</th>
        </tr></thead><tbody>
    `;
    for (let i=0;i<comps.length;i++){
      const c = comps[i];
      const sc = scoresObj && scoresObj[c.key] ? scoresObj[c.key] : {};
      const h = sc && sc.hifzScore != null ? sc.hifzScore : '';
      const a = sc && sc.ahkamScore != null ? sc.ahkamScore : '';
      const total = (h === '' && a === '') ? '' : (Number(h || 0) + Number(a || 0));
      tableHtml += `<tr>
  ...
  // Export ALL sheikhs in one file (grouped) — include office column per competitor and use scores
  async function exportAllSheikhs() {
    ...
    for (let idx=0; idx<sheikhKeys.length; idx++){
      ...
      const [compsSnap, scoresSnap] = await Promise.all([
        firebase.database().ref(`competitors/${username}`).once('value'),
        firebase.database().ref(`scores/${username}`).once('value')
      ]);
      const comps = [];
      ...
      tableHtml += `<table><thead><tr><th>م</th><th>اسم المتسابق</th><th>المنطقة</th><th>المكتب</th><th>مستوى الحفظ</th><th>حفظ /75</th><th>أحكام /25</th><th>الإجمالي</th></tr></thead><tbody>`;
      for (let i=0;i<comps.length;i++){
        const c = comps[i];
        const sc = scoresObj && scoresObj[c.key] ? scoresObj[c.key] : {};
        const h = sc && sc.hifzScore != null ? sc.hifzScore : '';
