// admin.js — لوحة تحكم القائد (مصحّح: جميع عمليات .once تستخدم 'value' بشكل صحيح)
// يجلب الشيوخ ثم المتسابقين من competitors/{username} ويتيح التعديل وتنزيل Excel.
// يتضمن إنشاء مشرف افتراضي إذا لم يكن موجوداً (غير آمن للاستخدام طويل الأمد — غيّره بعد الاستخدام).

(function(){
  // ---------- إعداد الاعتماد الافتراضي (غيّره فوراً بعد الدخول) ----------
  const DEFAULT_ADMIN = {
    username: 'admin',
    password: 'admin123'
  };
  ...
  // On load: create default admin if missing, then init UI
  (async function init(){
    try {
      if (window.firebase && firebase.database) {
      ...
      console.error('create default admin error', e);
    }
  ...
  async function createDefaultAdminIfMissing() {
    const username = String(DEFAULT_ADMIN.username).trim();
    if (!username || !window.firebase || !firebase.database) return;
    const ref = firebase.database().ref(`admins/${username}`);
    const snap = await ref.once('value');
    if (snap.exists()) return;
    const hash = await hashPassword(String(DEFAULT_ADMIN.password));
    await ref.set({
      passwordHash: hash,
      createdAt: firebase.database.ServerValue.TIMESTAMP,
      note: 'Default admin auto-created by client script. Change password and remove auto-create code after first use.'
    });
    notice(`تم إنشاء حساب المشرف الافتراضي باسم "${username}". غيّر كلمة المرور فورًا.`, '#0b6cf6');
  }
  ...
  adminLoginForm && adminLoginForm.addEventListener('submit', async (e) => {
    ...
    try {
      const adminSnap = await firebase.database().ref(`admins/${username}`).once('value');
      if (!adminSnap.exists()) {
        notice('اسم المستخدم غير موجود أو ليس لديك صلاحية.', '#d9534f');
        return;
      }
      const admin = adminSnap.val();
      const inputHash = await hashPassword(password);
      if (admin.passwordHash !== inputHash) {
        notice('كلمة المرور غير صحيحة.', '#d9534f');
        return;
      }
  ...
  async function toggleCompetitorsPanel(rowEl, username) {
    ...
    try {
      ...
      console.error(err);
      panel.querySelector('.sheikh-panel-inner').textContent = 'فشل جلب المتسابقين: ' + (err.message || String(err));
    }
  }
  ...
  async function exportSheikh(username) {
    // NOTE: use correct event type 'value' (lowercase) to avoid the error the user saw.
    const [compsSnap, resultsSnap, sheikhSnap] = await Promise.all([
      firebase.database().ref(`competitors/${username}`).once('value'),
      firebase.database().ref(`results/${username}`).once('value'),
      firebase.database().ref(`sheikhs/${username}`).once('value')
  ...
  async function exportAllSheikhs() {
    ...
    for (let idx=0; idx<sheikhKeys.length; idx++){
      ...
      tableHtml += `<table><thead><tr><th>م</th><th>اس�� المتسابق</th><th>المنطقة</th><th>مستوى الحفظ</th><th>حفظ /75</th><th>أحكام /25</th><th>الإجمالي</th></tr></thead><tbody>`;
      for (let i=0;i<comps.length;i++){
        const c = comps[i];
        const res = resultsObj && resultsObj[c.key] ? resultsObj[c.key] : {};
        const h = res && res.hifzScore != null ? res.hifzScore : '';
